...
AnswerLen = ir_measures.define_byquery(
    lambda qrels, res: len(res.iloc[0]["qanswer"]), name="AnswerLen", support_cutoff=False
)

AnswerZeroLen = ir_measures.define_byquery(
    lambda qrels, res: 1 if len(res.iloc[0]["qanswer"]) == 0 else 0, name="AnswerZeroLen", support_cutoff=False
)
...
def _rouge(
    measure: Literal["precision", "recall", "fmeaure"],
    type: Literal["rouge1", "rouge2", "rougeL"] = "rouge1",
...
def _bertscore(qrels, res, rel=3, submeasure="f1", agg="max"):
    ...
    results = _bertscore_model.compute(
        predictions=predictions, references=references, lang="en", model_type="bert-large-uncased", verbose=False
    )
...
def BERTScore(rel=3, submeasure: str = "f1", agg: str = "max"):
    ...
    return ir_measures.define_byquery(
        lambda qrels, res: _bertscore(qrels, res, rel=rel, agg=agg), name="BERTScore", support_cutoff=False
    )
