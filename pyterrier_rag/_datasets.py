...
class FlashRAGDataset(RAGDataset):
    ...
    def get_topics(self, variant: Optional[str] = None) -> pd.DataFrame:
        df = pd.read_json(
            "hf://datasets/RUC-NLPIR/FlashRAG_datasets/" + self.splits[variant],
            lines=True,
        )
    ...
    def get_answers(self, variant: Optional[str] = None) -> pd.DataFrame:
        ...
        return (
            df[["id", "golden_answers"]]
            .rename(columns={"id": "qid", "golden_answers": "gold_answer"})
            .explode("gold_answer")
        )  # explode deals with multiple answers
        ...
        with bz2.open(filename, "rt") as f:
            for lineno, line in enumerate(f):
                try:
                    ...
                    if lineno > 10:
                        warn(
                            "Ignoring JSON decoding error on line number %d, line %sm error %s"
                            % (lineno, line, str(jse))
                        )
                    else:
                        raise jse
        ...
        with pt.io.autoopen(filename, "r", encoding="utf-8", errors="replace") as f:
            ...
            for lineno, line in enumerate(f):
                try:
                    ...
                    if lineno > 10:
                        warn(
                            "Ignoring JSON decoding error in file %s on line number %d, line %s error %s"
                            % (filename, lineno, line, str(jse))
                        )
                    else:
                        raise RuntimeError(
...
def _2WikiMultihopQA_topics(self, component, variant):
    ...
    for id, idgroup in pt.tqdm(
        all_data.explode("context").groupby("_id"),
        desc="Reading 2WikiMultihopQA %s.json" % variant,
    ):
