...
class SearchO1(pt.Transformer):
    ...
    def tokenizer_encode(self, prompts: List[str]) -> List[torch.Tensor]:
        inputs = self.tokenizer(
            prompts, padding=True, truncation=True, max_length=self.generator.text_max_length, return_tensors="pt"
        )
    ...
    def generate(self, sequences: List[dict]) -> List[str]:
        ...
        token_ids = self.generator.model.generate(
            ...
            stopping_criteria=[
                StopWordCriteria(self.tokenizer, prompt_length, [END_SEARCH_QUERY, self.tokenizer.eos_token])
            ],
        )
    ...
    def analyze_docs(
        self, sequences: List[dict], prev_reasonings: List[str], queries: List[str], retrieval_results: List[List[dict]]
    ) -> List[str]:
        """
        ...
        for prompt, text, info in zip(prompts, generated_texts, extracted_infos):
            outputs.append({"prompt": prompt, "raw_output": text, "extracted_info": info})
        return outputs
        ...
        while True:
            ...
            if sequences_not_finished:
                ...
                if sequences_require_retrieval:
                    ...
                    for seq, query, retrieval_result, analysis in zip(
                        sequences_require_retrieval, queries, retrieval_results, doc_analyses
                    ):
    ...
    def generate(self, sequences: List[dict]) -> List[str]:
        ...
        for i, raw_prompt in enumerate(raw_prompts):
            ...
            if flag:
                continue
            if (
                generated_texts[i].rstrip().endswith(END_SEARCH_QUERY)
                or "\\boxed" in generated_texts[i]
                or "</think>" in generated_texts[i]
            ):
        ...
        if additional_retrieval_required_prompts:
            ...
            token_ids = self.generator.model.generate(
                ...
                stopping_criteria=[
                    StopWordCriteria(self.tokenizer, input_ids.shape[-1], [END_SEARCH_QUERY, self.tokenizer.eos_token])
                ],
            )
            generated_token_ids = token_ids[:, input_ids.shape[-1] :]
            additional_retrieval_required_generated_texts = self.tokenizer.batch_decode(
