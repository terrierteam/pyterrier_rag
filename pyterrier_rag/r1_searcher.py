...
class R1Searcher(pt.Transformer):
    ...
    def transform_iter(self, inp):
        ...
        for k in range(self.max_iterations):
            output = self.llm.generate([continued_answer["chat_prompt"]], self.sampling_params, use_tqdm=self.verbose)[
                0
            ]
            ...
            if "<answer>" in generated_text and stop_reason == "</answer>":
                ...
                if query:
                    ...
                    original_data = {
                        "chat_prompt": prompt
                        + generated_text.strip(),  # + "<|end_of_query|> "+ "\n\nThe retrieved content are:\n<tool_call>\n"  +  doc_content + "\n</tool_call>\n\n",
                    ...
                    continued_text_now["chat_prompt"] = (
                        continued_text_now["chat_prompt"]
                        + "<|end_of_query|>\n\n"
                        + "<|begin_of_documents|>\n"
                        + doc_content
                    ...
                    continued_text_now["gen_text_store"] = (
                        continued_text_now["gen_text_store"]
                        + "<|end_of_query|>\n\n"
                        + "<|begin_of_documents|>\n"
                        + doc_content
